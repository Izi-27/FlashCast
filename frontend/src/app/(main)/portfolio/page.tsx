...
import { Briefcase, TrendingUp, ArrowUpRight, ArrowDownRight, PieChart, Activity, DollarSign, Target } from "lucide-react";

const PortfolioPage = () => {
  const [selectedTimeRange, setSelectedTimeRange] = useState("7d");

  const portfolioStats = [
    { label: "Total Value", value: "$15,482.65", change: "+12.5%", icon: <DollarSign className="w-6 h-6" />, positive: true },
    { label: "24h Change", value: "+$1,234.50", change: "+8.7%", icon: <TrendingUp className="w-6 h-6" />, positive: true },
    { label: "Total Predictions", value: "156", change: "+23", icon: <Target className="w-6 h-6" />, positive: true },
    { label: "Success Rate", value: "76.3%", change: "+2.1%", icon: <Activity className="w-6 h-6" />, positive: true },
  ];

  const assetDistribution = [
    { name: "BTC", percentage: 45, value: "$6,967.19", color: "from-purple-500 to-purple-600" },
    { name: "ETH", percentage: 30, value: "$4,644.80", color: "from-pink-500 to-pink-600" },
    { name: "SOL", percentage: 15, value: "$2,322.40", color: "from-blue-500 to-blue-600" },
    { name: "Others", percentage: 10, value: "$1,548.26", color: "from-zinc-400 to-zinc-500" },
  ];

  const recentPredictions = [
    { id: 1, coin: "Bitcoin", type: "up", confidence: 85, timeFrame: "24h", status: "active", profit: "+$245" },
    { id: 2, coin: "Ethereum", type: "down", confidence: 72, timeFrame: "1w", status: "completed", profit: "+$180" },
    { id: 3, coin: "Solana", type: "up", confidence: 68, timeFrame: "12h", status: "active", profit: "+$95" },
    { id: 4, coin: "Cardano", type: "up", confidence: 91, timeFrame: "3d", status: "completed", profit: "+$320" },
  ];
          ...
          {portfolioStats.map((stat, index) => (
            ...
            >
              ...
              <div>
                ...
                <h3 className="text-2xl font-bold text-zinc-800 mt-1">{stat.value}</h3>
              </div>
            </motion.div>
          ))}
        </div>
          ...
          >
            ...
            <div className="space-y-3">
              {assetDistribution.map((asset, index) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-3 h-3 rounded-full bg-linear-to-br ${asset.color}`}></div>
                    <span className="font-medium text-zinc-800">{asset.name}</span>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-zinc-800">{asset.percentage}%</p>
          ...
          <div className="space-y-4">
            {recentPredictions.map((prediction, index) => (
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  <span className="font-bold text-green-600">{prediction.profit}</span>
